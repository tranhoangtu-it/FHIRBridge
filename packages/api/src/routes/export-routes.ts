...
import { postExportSchema, getExportStatusSchema, getExportDownloadSchema } from '../schemas/export-schemas.js';
...
export async function exportRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post<{ Body: ExportBody }>(
    ...
    async (request: FastifyRequest<{ Body: ExportBody }>, reply: FastifyReply) => {
      ...
      const userId = (request as unknown as { authUser?: { id: string } }).authUser?.id ?? 'anonymous';
  ...
  fastify.get<{ Params: IdParams }>(
    ...
    async (request: FastifyRequest<{ Params: IdParams }>, reply: FastifyReply) => {
      const userId = (request as unknown as { authUser?: { id: string } }).authUser?.id ?? 'anonymous';
      const record = exportService.getStatus(request.params.id, userId);
      if (!record) {
        return reply.status(404).send({ statusCode: 404, error: 'Not Found', message: 'Export not found' });
      }
      return reply.send({
  ...
  fastify.get<{ Params: IdParams }>(
    ...
    async (request: FastifyRequest<{ Params: IdParams }>, reply: FastifyReply) => {
      ...
      if (record.status !== 'complete' || !record.bundle) {
        return reply.status(409).send({ statusCode: 409, error: 'Conflict', message: `Export is ${record.status}` });
      }
